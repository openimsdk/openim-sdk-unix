...
export function verifyEnterpriseDriverInvariants(publicRoot: string, privateRoot: string): void {
  ...
  assertIncludes(
    ...
    [
      ...
      'requestedEpoch !== OpenIMHarmonyDriver.epoch',
      'const initEpoch: number = OpenIMHarmonyDriver.startSession()',
      'OpenIMHarmonyDriver.bindAllNativeEvents()',
      'OpenIMHarmonyDriver.completeStart(initEpoch, true)',
      'AppFramework.Native',
      'Platform.HarmonyOS',
      'static unInitSDK(',
      'const stoppingEpoch: number = OpenIMHarmonyDriver.beginShutdown()',
      'const priorBarrier: Promise<string> = OpenIMHarmonyDriver.teardownBarrier',
